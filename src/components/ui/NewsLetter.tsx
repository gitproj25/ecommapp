...
<section className="min-w-[clamp(220px,95%,1300px)] absolute left-1/2 bottom-0 transform -translate-x-1/2 translate-y-1/2 
 bg-black py-6 px-6  rounded-xl z-50 border border-green-600">
  ...
  <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 px-2">
    {/* Left text */}
    <h2 className=" font-integral leading-[1.1] font-extrabold text-[clamp(1.4rem,3.5vw,2.8rem)] text-white text-center md:text-left">
    ...
    <form className="flex flex-col sm:flex-col items-center gap-3 w-full md:w-auto">
      <div className="flex items-center bg-white rounded-full px-4 py-3 w-full sm:w-96">
        ...
        <input
          ...
          className="w-full text-[clamp(1.1rem,1.5vw,0.9rem)]  font-Satoshi font-light black outline-none bg-transparent"
          required
        />
      </div>
      <button
        type="submit"
        className="bg-white text-black text-[clamp(1.1rem,1.1vw,0.4rem)]  font-Satoshi font-Regular px-6 sm:px-8 py-2.5 rounded-full hover:bg-gray-200 transition w-full sm:w-96"
      >
