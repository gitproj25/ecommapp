...
export default function Header() {
 ...
 return (
    <header className="w-full border-b border-gray-200 py-1">
  <div className="max-w-[clamp(360px,92%,2140px)] mx-auto flex items-center justify-between px-4 py-3 lg:py-4">
  {/* Left side (Logo + Mobile Menu button) */}
  <div className="flex items-center space-x-3">
    <button
      className="lg:hidden"
  ...
  {/* Desktop Nav */}
  <nav className="hidden lg:flex space-x-6 text-gray-700 font-normal relative ml-8">
   
    <div
  className="relative"
  tabIndex={0} // make div focusable
  onBlur={() => setIsShopOpen(false)} // closes dropdown on losing focus
>
      <button
        className="flex items-center space-x-1 cursor-pointer"
        onClick={() => setIsShopOpen(!isShopOpen)}
      >
        <span className="text-[clamp(0.75rem,1.2vw,1.125rem)] font-satoshi font-light">
          Shop
        </span>
        <ChevronDown className="h-4 w-4" />
      </button>
    ...
    <Link
      href="/#on-sale"
      className="text-[clamp(0.75rem,1.2vw,1.125rem)] font-satoshi font-light"
    >
      On Sale
    </Link>
    <Link
  ...
  <div className="flex items-center space-x-3">
    ...
    <img src="/u.png" className="cursor-pointer h-6 w-6" />
  </div>
</div>
